<?php
require( dirname(__FILE__) . '/php/wp-load.php' );

if ( is_user_logged_in() ) {
	define('WP_USE_THEMES', true);
	require('./php/wp-blog-header.php');
} else {
	
	//here is the modified login page
...
<body class="login">
<?php   if ( !is_multisite() ) { ?>
<div id="login"><h1><a href="<?php echo apply_filters('login_headerurl', 'http://ored.net/'); ?>" title="<?php echo apply_filters('login_headertitle', __('An O Red Production')); ?>"><?php bloginfo('name'); ?></a></h1>
<?php   } else { ?>
...
case 'register' :
	if ( is_multisite() ) {
		...
		wp_redirect( apply_filters( 'wp_signup_location', get_bloginfo('wpurl') . '/php/wp-signup.php' ) );
		exit;
	}
...
<?php		exit;
		...
		if ( !$user->has_cap('edit_posts') && ( empty( $redirect_to ) || $redirect_to == '/php/' || $redirect_to == site_url() ) )
...
} // end of ored password protected site.
?>
