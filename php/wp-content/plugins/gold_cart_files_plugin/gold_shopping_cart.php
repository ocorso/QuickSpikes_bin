	...
	$output .= "<div class='wpcart_gallery'>";
		...
		if ($attachments) {
			foreach ($attachments as $post) {
				...
				$link = str_replace( 'a href' , 'a rev="' . $preview_link[0] . '" class="thickbox" rel="product_' . $product_id . '" href' , $link );
				$output .= $link;
			}
		}
				$output .= "</div>";
    ...
    if($seperator == "&amp;") {
      ...
      foreach($url_parameters as $url_parameter) {
		...
		if(get_option('show_live_search') == 1) {
			$output .= "  <input type='text' value='".$_GET['product_search']."' onkeyup='autocomplete(event)' autocomplete='off' name='product_search' class='wpsc_product_search wpsc_live_search' id='wpsc_search_autocomplete' />\n\r";
		} else {
